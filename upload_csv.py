...
def uploadgrades(course_id,grade_csv,mapping_csv,lti_key,
                 lti_secret,platform_url, endpoint):
    arguments['course_id']=course_id
    arguments['grade_csv']=grade_csv
    arguments['mapping_csv']=mapping_csv
    arguments['lti_key']=lti_key
    arguments['lti_secret']=lti_secret
    arguments['platform_url']=platform_url
    arguments['endpoint']=endpoint
    _doupload(arguments)
    
def _doupload(arguments):
    try:
        if arguments['mapping_csv']==None:
            mapping=None
        else:
        ...
        if False:
            ...
            for row in _generate_valid_grading_rows(arguments['grade_csv']):
                argarray.append((
                    ...
                    endpoint_url,
                    arguments['lti_key'],
                    arguments['lti_secret'],row))
            pool.map(_post_grade_arg,argarray)
    except _LTIToolError as error:
        print(error.message)
        exit()

def _post_grade_arg(arglist):
    _post_grade(*arglist)
...
def _main():
    ...
    """
    _doupload(_parse_command_line_arguments())
...
def _parse_command_line_arguments():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'The first column must be the student user or anon id, '
            'the second the student email address or assignment name, '
...
def _prompt_select_lti_endpoint(platform_url, course_id,choice_num=None):
    """
